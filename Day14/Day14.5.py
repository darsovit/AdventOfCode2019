...
def calcFuelPerOreUnits( input, units ):
    graph = buildWeightedGraph( input )
    print( graph )
    amountOfFuel = 1
    response = calcOrePerFuel( graph, amountOfFuel )
    amountOfFuel = int(units / response)
    while ( response < units ):
        print(response)
        amountOfFuel *= 2
        response = calcOrePerFuel( graph, amountOfFuel )
    lowerBound = int(amountOfFuel / 2)
    upperBound = amountOfFuel
    while lowerBound + 1 < upperBound:
        testAmount = lowerBound + int((upperBound - lowerBound) / 2)
        print(lowerBound, testAmount, upperBound)
        response = calcOrePerFuel( graph, testAmount )
        if response > units:
            upperBound = testAmount
        elif response < units:
            lowerBound = testAmount
    assert( calcOrePerFuel( graph, lowerBound ) <= units )
    return lowerBound

def calcOrePerFuel(graph, unitsOfFuel ):
    spares = {}
    required = {}
    required['FUEL'] = unitsOfFuel
    currentDepth = 0
...
def test():
    ...
    graph = buildWeightedGraph(input)
    assert( expected == calcOrePerFuel( graph, 1 ) )
    
test()
print( calcFuelPerOreUnits(readInput(), 1000000000000) )
